#include "callback_lifecycle.hpp"
...
bool wait_for_signaling_state_callback_test() {
    ...
    const bool should_dispatch = callback_should_dispatch;
...
Java_tel_schich_libdatachannel_PeerCallbackLifecycleTest_awaitSignalingStateCallback(JNIEnv* env, jclass clazz,
                                                                                    const jlong timeout_millis) {
    struct timespec deadline = deadline_after_millis(timeout_millis);
    pthread_mutex_lock(&callback_mutex);
    while (!callback_entered) {
        const int result = pthread_cond_timedwait(&callback_condition, &callback_mutex, &deadline);
        if (result == ETIMEDOUT) {
            pthread_mutex_unlock(&callback_mutex);
            return JNI_FALSE;
        }
